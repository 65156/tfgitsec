...
"""
    ...
    def format_reopen_comment(scan_date: Optional[str] = None) -> str:
        ...
        return f"""🔄 **Security Issue Detected Again**
...
Please review and address this security finding again."""
    ...
    def format_close_comment(scan_date: Optional[str] = None) -> str:
        ...
        return f"""🎉 **Security Issue Resolved**
...
*This issue was auto-closed by tfgitsec because the security finding no longer appears in the scan results.*"""
    ...
    def format_summary_comment(stats: dict, scan_date: Optional[str] = None) -> str:
        ...
        content = f"""## 📊 TfSec Security Scan Summary
...
### Findings by Severity"""
    ...
    @staticmethod
    def format_advisory_description(finding: TfSecFinding) -> str:
        """Format a TfSec finding into Security Advisory description markdown"""
        scan_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S UTC")
        
        content = f"""## Security Vulnerability in Terraform Configuration

**Rule**: {finding.rule_id} ({finding.long_id})
**Provider**: {finding.rule_provider.upper()}
**Service**: {finding.rule_service}
**Severity**: {finding.severity}
...
- **Resource**: `{finding.resource}`
- **File**: {finding.location.file_basename}
- **Lines**: {finding.location.line_range_str}
...
### Scan Information
- **Scan Date**: {scan_date}
- **Tool**: tfgitsec (automated security scanning)
- **Unique ID**: `{finding.unique_id}`

---
*This security advisory was automatically created by tfgitsec from TfSec security scan results.*
"""
        return content

    @staticmethod
    def format_advisory_reopen_comment(scan_date: Optional[str] = None) -> str:
        """Format comment for when reopening a resolved Security Advisory"""
        if scan_date is None:
            scan_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S UTC")
            
        return f"""🚨 **Security Vulnerability Detected Again**

This previously resolved security vulnerability has reappeared in the latest TfSec scan.

- **Scan Date**: {scan_date}
- **Status**: Automatically reopened by tfgitsec
- **Action Required**: Please review and address this security vulnerability again

This Security Advisory was reopened because the vulnerability is still present in the codebase."""

    @staticmethod
    def format_advisory_close_comment(scan_date: Optional[str] = None) -> str:
        """Format comment for when auto-closing a resolved Security Advisory"""
        if scan_date is None:
            scan_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S UTC")
            
        return f"""✅ **Security Vulnerability Resolved**

This security vulnerability is no longer detected in the latest TfSec security scan.

- **Scan Date**: {scan_date}
- **Status**: Automatically resolved
- **Resolution**: The security issue has been remediated

*This Security Advisory was auto-closed by tfgitsec because the security vulnerability no longer appears in the scan results.*"""
